        ...
        <Consumer>
            {value=>(
                <div className="flex bg-pink-700 h-20 w-full text-left justify-between p-3 text-white text-lg relative">
                    ...
                    <button className="absolute right-0 bottom-0 bg-white text-pink-700 text-center w-12 h-12 rounded-full border-2 border-transparent hover:bg-pink-100 -mb-5 mr-2 outline-none text-2xl" onClick={this.btnAddClickHandler.bind(this, value.dispatch)}>+</button>
                </div>
            )}
        </Consumer>
        )
